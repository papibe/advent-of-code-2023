...
func solution(filename string) int {
	...
	for hqueue.Len() > 0 {
		...
		if row == destination_row && col == destination_col && s_steps >= 4 {
			return heat
		}
		...
		for _, step := range STEPS {
			...
			if 0 <= new_row && new_row < len(city_map) && 0 <= new_col && new_col < len(city_map[0]) {
				...
				if (dir_row != 0 || dir_col != 0) && s_steps < 4 {
					if dir_row != step_row || dir_col != step_col {
						continue
					}
				}

				// if we already took 10 steps don't continue in this direction
				if step_row == dir_row && step_col == dir_col && s_steps >= 10 {
					continue
				}
...
func main() {
	fmt.Println(solution("./example1.txt")) // 94
	fmt.Println(solution("./example2.txt")) // 71
	fmt.Println(solution("./input.txt"))    // 1382
}
